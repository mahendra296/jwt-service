...
public class RoleController {
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    @Audited(index = 0, shouldStoreAll = true, activity = "VIEW_ROLE_DETAILS")
    ...
    public ResponseEntity<ApiResponse<Role>> updateRole(@PathVariable Long id, @RequestBody Role roleDetails) {
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    @Audited(index = 0, shouldStoreAll = true, activity = "DELETE_ROLE")
